...
class DetectionModel(object):                                   # 2
    ...
    def detection(self, dataset, respath):
        ...
        for i in range(0, len(dataset)):
            ...
            # blur
            if ishuge:
                targetImage = self.medianFilter(targetImage, ishuge)
                baseImage = self.medianFilter(baseImage, ishuge)
    ...
    def drawRectangle(self, image, ratio, baseImage, name, ishuge=False, hugewidthlowthresh=0.1,     # TODO change 0.15->0.1
                      hugewidthhighthresh=0.91, hugeheightlowthresh=0.08, hugeheighthighthresh=0.9,
        ...
        for c in cnts:
            if not ishuge or cv.contourArea(c) > thresh:
                ...
                if littleDiff or widthlowthresh * width < w < widthhighthresh * width\
                    ...
                    if not (ishuge and not (
                             (0.1 * width < center_x < 0.92 * width) and (0.1 * height < center_y < 0.92 * height))):    # TODO
...
# VCjjoHW3VOrZHEfmCq/SiJeW4ed0xnFhjDkIV4ExKJqEkRLbj+wcWdUyjLpRxqRk 84.41 points
# 7Z+worlGfyHFddAiXKILarZVMuNC23rBG02RPlTSeprEk0jiTji2ohXfAUcNHsaW 84.15 points
