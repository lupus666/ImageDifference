...
class DetectionModel(object):                                   # 2
    ...
    def detection(self, dataset, respath):
        ...
        for i in range(0, len(dataset)):
            ...
            baseImage = baseImage[1: height - 1, 1: width - 1, :]
            targetImage = targetImage[1: height - 1, 1:width - 1, :]
            height, width, channels = baseImage.shape
            ...
            if 600000 > height * width > self.hugeImageThresh:
                isblur = True

            # blur
            if isblur:
                targetImage = self.medianFilter(targetImage, ishuge, 0.005)
                baseImage = self.medianFilter(baseImage, ishuge, 0.005)
            # # if self.showImage:
            #     cv.imshow('medianblur1', baseImage)
            #     cv.imshow('medianblur2', targetImage)
            ...
            # if ishuge:
            #     deEdge_start_time = time.time()
            #     baseImage = self.deEdge(baseImage, True)
            #     targetImage = self.deEdge(targetImage, True)
            #     deEdge_end_time = time.time()
            #     self.deEdge_time += deEdge_end_time - deEdge_start_time
            ...
            thresh = self.thresholdFilter(diff, ishuge, isblur)
            ...
            items = self.drawRectangle(thresh, (height_ratio1, width_ratio1), baseImage, name, ishuge, isblur=isblur)
    ...
    def deEdge(self, image, isSecond=False):
        ...
        deedgedImg = image.astype(np.float32) - edge1.astype(np.float) * self.edgeX - edge2.astype(
            np.float) * self.edgeY
        if isSecond:
            deedgedImg = image.astype(np.float32) - edge1.astype(np.float) * 0.5 - edge2.astype(
                np.float) * 0.05
    ...
    def thresholdFilter(self, image, ishuge=False, isblur=False):
        ...
        if isblur and len(cnts) < 150:
            thresh = cv.adaptiveThreshold(thresh, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY_INV, 5, 3)
    ...
    def medianFilter(selfs, image, ishuge, proportion=0.03):    # TODO Adaptive median filter
        # MedianFilter
        [height, width, channels] = image.shape
        size = int(height * 0.0) + (int(height * 0.03) - 1) % 2
        if ishuge:
            size = int(height * proportion) + (int(height * proportion) - 1) % 2

        blur = cv.medianBlur(image, size)      # TODO change 3 to 5

        return blur

    def drawRectangle(self, image, ratio, baseImage, name, ishuge=False, isblur=False, hugewidthlowthresh=0.1,     # TODO change 0.15->0.1
                      hugewidthhighthresh=0.95, hugeheightlowthresh=0.15, hugeheighthighthresh=0.95,     # TODO change w0.91->0.95 h0.90->0.95
        ...
        thickness = 3 - round(len(cnts) / 100)
        if ishuge:
            thickness = 4 - round(len(cnts) / 20)   # 5 100 TODO 60
        if isblur:
            thickness = 1
        res = []
        ...
        for c in cnts:
            ...
            if w < 0.9 * width and h < 0.9 * height:
                ...
                if not (ishuge and not (                            # TODO 0.10->0.08
                        (0.10 * width < center_x < 0.95 * width) and (0.08 * height < center_y < 0.95 * height))):
                    x1 = x
                    y1 = y
                    x2 = x + w
                    y2 = y + h
        ...
        for c in cnts:
            ...
            if littleDiff or w > self.rectEdgeThresh * width and h > self.rectEdgeThresh * height:
                if not (ishuge and not (                            # TODO 0.10->0.08   0.95->0.92 0.92->0.95
                        (0.10 * width < center_x < 0.95 * width) and (0.08 * height < center_y < 0.95 * height))):
                    x1 = x
                    y1 = y
                    x2 = x + w
            ...
            for c in cnts:
                ...
                if littleDiff or widthlowthresh * width < w < widthhighthresh * width \
                    ...
                    if not (ishuge and not (
                        ...
                        x2 = x + w
                        y2 = y + h
                        cv.rectangle(blank, (x1, y1), (x2, y2), (0, 0, 255), 1)    # TODO 2 change to 3 to 1
            if self.showImage:
        ...
        if len(cnts) >= 4:
            ...
            # print(cnts)
            # print(std)
        # TODO Ratio between width and height
        for c in cnts:
            if not ishuge or cv.contourArea(c) > thresh:
                ...
                if littleDiff or widthlowthresh * width < w < widthhighthresh * width\
                        and heightlowthresh * height < h < heighthighthresh * height or \
                        ((ishuge and (width * 0.3 < center_x < width * 0.7 and height * 0.3 < center_y < height * 0.7) and
                         w * h >= width * height * 0.0030)) or \
                        (not ishuge and (width * 0.1 < center_x < width * 0.9 or height * 0.1 < center_y < height * 0.9) and    # TODO change 0.3->0.1, 0.7->0.9
                         w * h >= width * height * 0.0030):     # TODO change 0.0025 -> 0.0015
                    if not (ishuge and not (
                             (0.05 * width < center_x < 0.95 * width) and (0.1 * height < center_y < 0.90 * height))):    # TODO change 0.90->0.95
        ...
        if len(res) == 0 and not self.flag2:
            self.flag2 = True
            res = self.drawRectangle(image, ratio, baseImage, name, ishuge, hugewidthhighthresh=0.95,
...
if __name__ == "__main__":
    ...
    # dataset = ImageDataSet("./test2_samples")
    dataset = ImageDataSet("./test2")
    # dataset = ImageDataSet("./impuse/")
    # dataset = ImageDataSet("./try/")
    # dataset = ImageDataSet("./small/")
    model = DetectionModel(0.7, 0.7, showImage=False, pause=False, nFeatures=2000, hugeImageThresh=200000, calibrate=True,
                           normalBlurSize=0.015,
                           firstBlurSize=0.035, secondBlurSize=0.03,   # 25 15
                           normalThreshSize=3, normalThreshC=2.5,
                           hugeThreshSize=7, hugeThreshC=4.5     # 9 4.5   TODO:5 2.5
                           )
    model.detection(dataset, './detections/')
    # model.detection(dataset, './impuse_res/')
