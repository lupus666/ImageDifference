...
class DetectionModel(object):                                   # 2
    ...
    def detection(self, dataset, respath):
        ...
        for i in range(0, len(dataset)):
            ...
            baseImage = baseImage[2:height-2, 2:width-2, :]
            targetImage = targetImage[2:height-2, 2:width-2, :]
            height, width, channels = baseImage.shape
    ...
    def thresholdFilter(self, image, ishuge=False):
        ...
        if len(cnts) <= 100:
            thresh = cv.adaptiveThreshold(thresh, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY_INV, 7, C)
    ...
    def drawRectangle(self, image, ratio, baseImage, name, ishuge=False, hugewidthlowthresh=0.1,     # TODO change 0.15->0.1
            ...
            for c in cnts:
                ...
                if littleDiff or widthlowthresh * width < w < widthhighthresh * width \
                    ...
                    if not (ishuge and not (
                        ...
                        x2 = x + w
                        y2 = y + h
                        cv.rectangle(blank, (x1, y1), (x2, y2), (0, 0, 255), 2)    # TODO 2 change to 3 to 1
            if self.showImage:
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.7, 0.7, showImage=True, pause=True, nFeatures=2000, hugeImageThresh=200000, calibrate=True,
