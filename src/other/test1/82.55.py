        ...
        for c in cnts:
            ...
            if littleDiff or w > self.rectEdgeThresh * width and h > self.rectEdgeThresh * height:
                if not (ishuge and not (
                        (0.10 * width < center_x < 0.95 * width) and (0.10 * height < center_y < 0.90 * height))):
                    x1 = x
                    y1 = y
                    x2 = x + w
                    y2 = y + h
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.6, 0.6, showImage=False, pause=False, nFeatures=2000, hugeImageThresh=300000,
