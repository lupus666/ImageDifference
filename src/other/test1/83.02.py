...
class DetectionModel(object):                                   # 2
    ...
    def drawRectangle(self, image, ratio, baseImage, name, ishuge=False, hugewidthlowthresh=0.15,
        ...
        # if len(cnts) == 0 and not self.flagone:
        #     self.flagone = True
            ...
            for c in cnts:
                ...
                if littleDiff or 0.05 * width < w < 0.95 * width and 0.05 * height < h < 0.95 * height or \
                    ...
                    if not (ishuge and not (
                        ...
                        x2 = x + w
                        y2 = y + h
                        cv.rectangle(blank, (x1, y1), (x2, y2), (0, 0, 255), 2)    # TODO change to 2
            if self.showImage:
        ...
        if len(res) == 0 and not self.flagtwo:
            self.flagtwo = True
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.6, 0.6, showImage=False, pause=False, nFeatures=2000, hugeImageThresh=200000,
                           normalBlurSize=0.015,
                           firstBlurSize=0.03, secondBlurSize=0.03,   # 25 15
