...
class Calibrator(object):
    ...
    def siftImageAlignment(self, img1, img2, method='sift'):
        ...
        if len(goodMatch) > 4:
            ...
            # if method == 'sift':
            #     if 0.5 >= H[0, 2] >= -0.5 and 0.5 >= H[1, 2] >= -0.5:
            #         imgOut, H, status = self.siftImageAlignment(img1, imgOut, 'other')
...
class DetectionModel(object):
    ...
    def detection(self, dataset, respath):
        ...
        for i in range(0, len(dataset)):
            ...
            baseImage = self.medianFilter(baseImage, ishuge, 0.015)
            targetImage = self.medianFilter(targetImage, ishuge, 0.015)
            # Gaussian filtering
            if ishuge:
                gaussian_start_time = time.time()
                baseImage = self.gaussionFilter(baseImage, ishuge, 0.015)
                targetImage = self.gaussionFilter(targetImage, ishuge, 0.015)
            ...
            #baseImage=denoise_tv_chambolle(baseImage, weight=0.2,multichannel=True)
            #targetImage = denoise_tv_chambolle(targetImage, weight=0.2,multichannel=True)
            gaussian_end_time = time.time()
            gaussian_time += gaussian_end_time - gaussian_start_time
            if self.showImage:
    ...
    def morphology(self, image):
        ...
        thresh = cv.morphologyEx(image, cv.MORPH_OPEN, kernel, iterations=self.morphologyIter, borderType=cv.BORDER_CONSTANT)
    ...
    def gaussionFilter(self, image, ishuge, proportion=0.03):
        # Gauss filtering
        [height, width, channels] = image.shape
        size = int(height * 0.01) + (int(height * 0.01) - 1) % 2
        if ishuge:
    ...
    def medianFilter(selfs, image, ishuge,proportion=0.03):
        # MedianFilter
        [height, width,channels] = image.shape
        size = int(height * 0.01) + (int(height * 0.01) - 1) % 2
        if ishuge:
    ...
    def drawRectangle(self, image, ratio, baseImage, ishuge=False):
        ...
        # The third iteration
        blank2 = cv.cvtColor(blank, cv.COLOR_BGR2GRAY)
        blank3 = np.zeros([int(height), int(width), 3], np.uint8)
        cnts = cv.findContours(blank2, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        cnts = cnts[1] if imutils.is_cv3() else cnts[0]
        for c in cnts:
            x, y, w, h = cv.boundingRect(c)
            ...
            if littleDiff or 0.05 * width < w < 0.95 * width and 0.05 * height < h < 0.95 * height or \
                ...
                if not (ishuge and not (
                    ...
                    #x2 = int((x + w) * ratio[1])
                    #y2 = int((y + h) * ratio[0])
                    cv.rectangle(blank3, (x, y), ((x + w), (y + h)), color=(0, 0, 255), thickness=-1)
                    #print("     ", x1, y1, x2, y2)
        ...
        blank2 = cv.cvtColor(blank3, cv.COLOR_BGR2GRAY)
        blank4 = np.zeros([int(height), int(width), 3], np.uint8)
        cnts = cv.findContours(blank2, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        cnts = cnts[1] if imutils.is_cv3() else cnts[0]
        expendRate=0.018
        edgeThresh=0.01
        ...
        for c in cnts:
            n1 = n1 +1
            x, y, w, h = cv.boundingRect(c)
            expandw = int(w * expendRate)
            expandh = int(h * expendRate)
            if (w > edgeThresh * width and h > edgeThresh * height):
                x1 = x - expandw if (x - expandw >= 0) else x
                y1 = y - expandh if (y - expandh >= 0) else y
                x2 = x + w + expandw if (x + w + expandw < width) else x + w
                y2 = y + h + expandh if (y + h + expandh < height) else y + h
                cv.rectangle(blank4, (x1, y1), (x2,y2), color=(0, 0, 255), thickness=-1)

        blank5 = cv.cvtColor(blank4, cv.COLOR_BGR2GRAY)
        cnts = cv.findContours(blank5, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        cnts = cnts[1] if imutils.is_cv3() else cnts[0]
        n2 = 0
        for c in cnts:
            n2 = n2+1
        if (n2>=n1):
            blank5 = cv.cvtColor(blank3, cv.COLOR_BGR2GRAY)
            cnts = cv.findContours(blank5, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
            cnts = cnts[1] if imutils.is_cv3() else cnts[0]
        for c in cnts:
            x, y, w, h = cv.boundingRect(c)
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.6, 0.6, showImage=False, pause=False, nFeatures=2000)
    model.detection(dataset, './detections/')
    # model.detection(dataset, './test_res/')
    cv.waitKey(0)
