...
class DetectionModel(object):                                   # 2
    ...
    def colorDifference(self, image1, image2):
        ...
        if np.mean(diff) < np.mean(diff6):
            diff = diff6
        res = diff1 * 0.33 + diff2 * 0.33 + diff3 * 0.33 + diff * 0.01
        mean = np.mean(res)
        res_n = res - mean
        res_n[res_n < 0] = 0
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.6, 0.6, showImage=False, pause=False, nFeatures=2000, hugeImageThresh=300000,  # TODO
                           normalBlurSize=0.014,
                           firstBlurSize=0.018, secondBlurSize=0.020,   # 25 15
