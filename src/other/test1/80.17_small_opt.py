...
class DetectionModel(object):                                   # 2
    ...
    def colorDifference(self, image1, image2):
        ...
        diff5 = cv.absdiff(g1 - r1, g2 - r2)
        diff6 = cv.absdiff(b1 - g1, b2 - g2)
        # 各个色差加权求和
    ...
    def drawRectangle(self, image, ratio, baseImage, name, ishuge=False):
        ...
        if ishuge:
            thickness = 5 - round(len(cnts) / 10)   # 5 100 TODO 60
        res = []
...
if __name__ == "__main__":
    ...
    model = DetectionModel(0.6, 0.6, showImage=True, pause=True, nFeatures=2000, hugeImageThresh=300000,  # TODO
                           normalBlurSize=0.014, morphologyIter=1, rectEdgeThresh=0.03,
                           firstBlurSize=0.022, secondBlurSize=0.015,   # 25 15
                           normalThreshSize=3, normalThreshC=2.5,
                           hugeThreshSize=7, hugeThreshC=2.5     # 9 4.5   TODO:5 2.5
                           )
